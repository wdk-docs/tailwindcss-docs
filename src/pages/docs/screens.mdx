...
## <Heading hidden>配置自定义屏幕</Heading>

在`主题`中定义项目的断点。显示`tailwind.config.js`文件中的'部分。键变成你的[响应式修饰符](/docs/responsive-design)(如`md:text-center`)，值是断点应该开始的`min-width`。
...
您可以随意设置尽可能多或尽可能少的屏幕，以您喜欢的方式命名它们。

### 覆盖默认值

要完全替换默认的断点，直接在`theme`键下添加你的自定义`screens`配置:
