---
title: 处理悬停、焦点和其他状态
shortTitle: 悬停、聚焦和其他状态
description: 使用实用工具对悬停、聚焦等元素进行样式设置。
---

import { TipGood, TipBad, TipInfo } from '@/components/Tip'
import { SnippetGroup } from '@/components/SnippetGroup'

Tailwind中的每个实用程序类都可以 _有条件_ 地应用，方法是在类名的开头添加一个修饰符，该修饰符描述您想要达到的条件。

例如，要在hover上应用`bg-sky-700`类，使用`hover:bg-sky-700`类:
