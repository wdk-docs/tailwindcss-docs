---
title: 函数和指令
description: 一个自定义函数和指令的引用，顺风向你的CSS公开。
---
...
指令是自定义的顺风特定的[at-rules](https://developer.mozilla.org/en-US/docs/Web/CSS/At-rule)，你可以在你的CSS中使用，为顺风CSS项目提供特殊的功能。
...
<TipBad>Don't put <code>@config</code> before your <code>@import</code> statements</TipBad>
...
<TipGood>Put your <code>@import</code> statements before the <code>@config</code> directive</TipGood>
...
Tailwind添加了一些自定义函数，您可以在CSS中使用它们来访问Tailwind特定的值。这些函数在构建时求值，并在最终的CSS中被静态值取代。

### theme()

使用`theme()`函数使用点表示法访问您的Tailwind配置值。
