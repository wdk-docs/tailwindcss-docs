---
title: 添加自定义样式
description: 向Tailwind添加自定义样式的最佳实践。
---

import { TipGood, TipBad, TipCompat, TipInfo } from '@/components/Tip'
import { SnippetGroup } from '@/components/SnippetGroup'

通常，使用框架时最大的挑战是，当你需要一些框架无法处理的事情时，弄清楚你应该做什么。

顺风从一开始就被设计为可扩展和可定制的，所以无论你在构建什么，你都不会觉得你在与框架作斗争。

本指南涵盖了自定义设计令牌，如何在必要时打破这些限制，添加自己的自定义CSS以及使用插件扩展框架等主题。
...
在[插件](/docs/plugins)文档中了解更多关于编写自己的插件的信息。
