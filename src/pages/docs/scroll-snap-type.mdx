---
title: '滚动捕捉类型'
description: '用于控制在捕捉容器中强制执行捕捉点的严格程度的实用程序。'
---
...
使用`snap-x`实用程序可以在元素内启用水平滚动捕捉。

要使滚动捕捉工作，还需要在元素内的子元素上设置[滚动捕捉对齐](/docs/scroll-snap-align)。
...
使用`snap-mandatory`实用程序可以强制捕捉容器始终停留在捕捉点上。
...
使用`snap-proximity`实用程序可以使捕捉容器停靠在接近的捕捉点上。这是浏览器的默认设置。
