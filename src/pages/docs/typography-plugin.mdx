...
官方的顺风CSS排版插件提供了一组`prose`类，你可以用它来为任何你无法控制的香草HTML添加漂亮的排版默认值，比如从Markdown呈现的HTML，或者从CMS提取的HTML。

<a href="https://play.tailwindcss.com/uj1vGACRJA?layout=preview">
  <img src={require('@/img/typography-demo.jpg').default.src} alt="Tailwind CSS IntelliSense extension for Visual Studio Code" width={3086} height={1658} className="rounded-lg ring-1 ring-black/5" />
</a>
...
看看它看起来像什么行动，检查我们的[现场演示](https://play.tailwindcss.com/uj1vGACRJA?layout=preview)在Tailwind Play。
...
现在你可以使用`prose`类为任何香草HTML添加合理的排版样式:
...
Read the Tailwind CSS documentation on [ordering stacked modifiers](/docs/hover-focus-and-other-states#ordering-stacked-modifiers) to learn more.
...
Customizations should be applied to a specific modifier like `DEFAULT` or `xl`, and must be added under the `css` property. Customizations are authored in the same [CSS-in-JS syntax](/docs/plugins#css-in-js-syntax) used to write Tailwind plugins.
